...
class ChatGUI(tk.Tk):
    def __init__(self):
        ...
        # Video generation parameters frame (initially hidden)
        ...
        self.video_duration_var = tk.IntVar(value=4)
    ...
    def _call_video_api_threadsafe(self):
        ...
        try:
            ...
            if not prompt:
                self._on_api_done(False, "Error: Video prompt cannot be empty.")
                return
            ...
            if result["success"]:
                ...
                filename = f"generated_video_{timestamp}.mp4" # Assuming MP4 format
                video_path = self._save_video(result["video_data"], filename)

                append_message(
                    self.current_conv,
                    "assistant",
                    f"Generated video using {model} based on: '{prompt}' (Duration: {duration}s, Aspect Ratio: {aspect_ratio}). Saved to: {video_path}"
                )
    ...
    def _save_video(self, video_data, filename):
        ...
        Args:
            video_data (bytes): The video data in bytes
